...
>도메인 특화 언어. 특정 분야에 최적화된 프로그래밍 언어이고 해당 분야 또는 도메인의 개념과 규칙을 사용한다.  
ex) SQL, 정규식

### API 에서 DSL 로

>목표는 코드의 가독성과 유지 보수성을 가장 좋게 유지하는 것.

모든 클래스는 마치 라이브러리처럼 다른 클래스에게 자신과 상호작용할 수 있는 가능성을 제공한다.  
11장 에서는 API 에서 한걸음 더 나아가 DSL 구축을 도와주는 코틀린 기능을 설명한다.  
