...
동시성을 무력화하는 것이 불가능하며, 외부에서 락(Lock)을 걸면 오히려 속도가 더 느려진다. (내부에 이미 syncronizer가 있음)
...
예를 들면 `putIfAbsent(key, value)`는 Map의 디폴트 메서드인데 주어진 키에 맵핑된 값이 아직 없을 때만 새 값을 집어넣는다.
