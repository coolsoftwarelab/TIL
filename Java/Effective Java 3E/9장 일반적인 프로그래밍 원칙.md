...
철자 규칙은 직관적이지만 문법 규칙은 모호하다. 융통성있게 상식이 이끄는 대로 따르자.

### 패키지와 모듈 이름

- 조직의 인터넷 도메인 이름을 역순으로 사용한다.
- 예외 적으로 표준 라이브러리와 선택적 패키지들은 각각 java와 javax로 시작한다.
- 패키지 이름의 나머지는 해당 패키지를 설명하는 하나 이상의 요소로 이루어짐 8자 이하의 짧은 단어로 표현한다.
```
...
## 2. 문법 명명규칙
>철자 규칙과 비교하면 문법 규칙은 더 유연하고 논란도 많다.
...
자바빈즈 등에서 오랫동안 사용되어온 규칙으로, 이 규칙을 따라도 상관없다
<br>
