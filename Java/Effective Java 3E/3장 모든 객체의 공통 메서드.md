...
3. 대칭성(symmetry) : null이 아닌 모든 참조 값 x, y에 대해, x.equals(y)가 true면 y.equals(x)도 true다.
4. 추이성(transitivity) : null이 아닌 모든 참조 값 x, y, z에 대해, x.equals(y)가 true이고, y.equals(z)도 true면 x.equals(z)도 true다.
5. 일관성(consistency) :  null이 아닌 모든 참조 값 x, y에 대해, x.equals(y)를 반복해서 호출하면 항상 true이거나 false다.
6. null-아님 : null이 아닌 모든 참조 값 x에 대해, x.equals(null)은 false다.
...
참고
- 논리적 동치 : 객체의 기준이되는 값이 같다. `equals`
- 물리적 동치 : 참조하는 객체의 주소값이 같다. `==`
