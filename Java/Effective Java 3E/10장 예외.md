...
- 검사예외(Checked Exception)는 항상 따로따로 선언하고, 각 예외가 발생하는 상황을 javadoc의 @throw 태그를 사용하여 정확히 문서화
```java
/**
 *
...
(JVM 말고 Exception을 처리할 디른 주체가 발생할 수 없다는 의미. JVM이 알아서 예외를 처리해서 콘솔에 에러를 출력한다)
