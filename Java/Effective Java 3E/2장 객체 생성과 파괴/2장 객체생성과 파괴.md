...
Integer number = 1;
```
값이 1인 Integer 객체를 참조하는 `number` 변수

2. 부드러운 참조(Soft Reference)
```java
SoftReference<Integer> softRef = new SoftReference<Integer>(number)
```
number가 null 이라면 JVM이 메모리가 부족할 경우에 GC

3. Weak Referece(약한참조)
```java
