...
## 기타
### 참조 지역성(Locality of reference) = 지역성의 원리(principle of locality)

>어떤 데이터가 참조된 그 지역 및 시간 근처에서 다시 참조될 가능성이 높다는 경향

- 시간 지역성
  - 한 지점에서 특정 메모리 위치가 참조되면 가까운 장래에 동일한 위치가 다시 참조될 가능성이 높다 (ex. 반복문안의 데이터)
- 공간 지역성 
  - 특정 저장 위치가 특정 시간에 참조되면 가까운 미래에 가까운 메모리 위치가 참조될 가능성이 높다 (ex. 배열 내 요소)

![](https://itwiki.kr/images/b/bb/%EC%A7%80%EC%97%AD%EC%84%B1_%EA%B4%80%EC%B8%A1.png)

#### 메모리 계층 구조 사용의 효율성
- 임의 액세스 메모리 가 프로그래머에게 언제 어디서나 읽고 쓸 수 있는 능력을 제공하지만,  
  실제로 대기 시간과 처리량은 참조의 지역성을 증가시켜 개선되는 캐시의 효율성에 의해 영향을 받는다

즉, 캐쉬에서 코드나 데이터를 읽을 확률이 높을 수록(참조 지역성이 클수록) 성능이 증가
